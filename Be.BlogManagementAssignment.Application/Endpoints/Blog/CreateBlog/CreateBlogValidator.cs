...
public class CreateBlogValidator : AbstractValidator<CreateBlogRequest>
{
    public CreateBlogValidator()
    {
        RuleFor(x => x.Title)
            ...
            .MaximumLength(100).WithMessage("Title must not exceed 250 characters.");

        RuleFor(x => x.Slug)
            .NotEmpty().WithMessage("Slug is required.")
            .MaximumLength(100).WithMessage("Slug must not exceed 300 characters.")
            .Matches(@"^[a-z0-9]+(?:-[a-z0-9]+)*$")
            .WithMessage("Slug may only contain lowercase letters, digits, and hyphens.");

        RuleFor(x => x.ShortDescription)
            .MaximumLength(150).WithMessage("Short description must not exceed 500 characters.")
        ...
        RuleFor(x => x.ThumbnailUrl)
            .MaximumLength(200).WithMessage("Thumbnail URL must not exceed 2000 characters.")
